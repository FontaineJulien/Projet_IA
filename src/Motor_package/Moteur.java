 ...
 * 
 * En cours :
 * 		- Implémenter l'inférence par paquet de règles
 */
...
public class Moteur {
	 ...
	 */
	public boolean deepForward(Fait... goals){
		history.clear();
		List<Fait> goals_list = Arrays.asList(goals);
		return deepForward(goals_list);
	}
	
	public boolean deepForward(List<Fait> goals){
	 ...
	 */
	public boolean wideForward(Fait... goals){
		List<Fait> goals_list = Arrays.asList(goals);
		return wideForward(goals_list);
	}
	
	public boolean wideForward(List<Fait> goals){
		ArrayList<Boolean> activable = initActivable(); // Permet de savoir si la regle i a deja été activée => Une règle se déclenche au plus une fois
		boolean inf = true; // Permet de savoir si une inférence a été effectuée
		boolean dec; // Permet de savoir si la règle courrante est déclenchable
		boolean goal_reached=false; // Permet de savoir de savoir si le but donné à été atteint
	...
	private boolean ask(Fait goal){
		...
		System.out.println("Voulez vous faire entrer le but \""+goal_string+"\" dans la base de faits ? (Oui/Non)");
		...
		while(keep_asking){
			...
			if(answer.toLowerCase().equals("oui") || answer.toLowerCase().startsWith("o")){
				...
				if(answer.toLowerCase().equals("non") || answer.toLowerCase().startsWith("n")){
					...
					System.out.println("La réponse n'est pas valide. Voulez vous faire entrer le but \""+goal_string+"\" dans la base de faits ? (Oui/Non)");
				}
			}
	...
	/**
	 * Ajouter un fait dans la base de Faits.
	 * Si la premiere lettre de fait est ! alors le fait ajouté aura la valuation false.
	 * @param fait -> la chaine du fait à ajouter
	 ...
	 * @param current_antecedents -> liste de String des antécédents
	 * @param current_consequences -> liste de String des conséquences
	 * @param Categorie				-> liste de String des catégories.
	...
	/**
	 * Renvoyer la liste des faits de la base de faits sous forme d'une liste de string
	 * @return
	 */
	...
	private void writeSystem(){
	    ...
	    try {
	      ...
	      //ECRITURE DU FICHIER
	      dos.writeBytes("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?>");	//L'ENTETE DU FICHIER
	      dos.writeBytes("\n<System>");
	      ...
	      for(int i=0;i<rule_list.size();i++){												//POUR CHAQUE REGLE
	    	  dos.writeBytes("<regle>\n");
		    	  ...
		    	  for(int j=0;j<ante.size();j++){												//POUR CHAQUE ANTECEDENT
		    		  dos.writeBytes("<antecedent value=\""+ante.get(j).getValuation()+"\">"+ante.get(j).getLabel()+"</antecedent>\n");
		    	  }
		    	  dos.writeBytes("</antecedents>\n");
		    	  List<Fait> cons = rule_list.get(i).getConsequences();
		    	  dos.writeBytes("<consequences>\n");
		    	  for(int j=0;j<cons.size();j++){												//POUR CHAQUE Consequence
		    		  dos.writeBytes("<consequence value=\""+cons.get(j).getValuation()+"\">"+cons.get(j).getLabel()+"</consequence>\n");
		    	  }
		    	  dos.writeBytes("</consequences>\n");
	    	  dos.writeBytes("</regle>\n");	    	  
	      }
	      ...
	      for(int i=0;i<fact_list.size();i++){	
	    	  dos.writeBytes("<fait value=\""+fact_list.get(i).getValuation()+"\">"+fact_list.get(i).getLabel()+"</fait>\n");
	      }
	      dos.writeBytes("</faits>\n");
